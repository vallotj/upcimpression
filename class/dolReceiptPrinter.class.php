                    ...
                    case 'DOL_PRINT_OBJECT_LINES':
                        foreach ($object->lines as $line) {
                            if (!empty($line->fk_parent_line)){
                                $spacestoadd = $nbcharactbyline - 2 - mb_strlen(strip_tags(htmlspecialchars_decode($line->product_label)))  - strlen($line->qty) - 10 - 10 - 1;
                                $spaces = str_repeat(' ', $spacestoadd > 0 ? $spacestoadd : 0);
                                $this->printer->text(
                                    strip_tags(htmlspecialchars_decode('  ' . $line->product_label )) .
                                    $spaces .
                                    $line->qty .
                                    str_pad(price(number_format($line->subprice + ($line->subprice * ($line->tva_tx / 100)), 2)), 10, ' ', STR_PAD_LEFT)
                                    . ' ' .
                                    str_pad(price($line->total_ttc), 10, ' ', STR_PAD_LEFT). "\n");
                            }
                            elseif ($line->fk_product) {
                                $this->printer->setEmphasis(true);
                                $spacestoadd = $nbcharactbyline - mb_strlen(strip_tags(htmlspecialchars_decode($line->product_label))) - strlen($line->qty) - 10 - 10 - 1;
                                $spaces = str_repeat(' ', $spacestoadd > 0 ? $spacestoadd : 0);
                                $this->printer->text(
                                    strip_tags(htmlspecialchars_decode($line->product_label)) .
                                    $spaces .
                                ...
                                $spacestoadd = $nbcharactbyline - mb_strlen($line->description) - mb_strlen($line->qty) - mb_strlen($line->subprice) - 10 - 1;
                                $spaces = str_repeat(' ', $spacestoadd > 0 ? $spacestoadd : 0);
